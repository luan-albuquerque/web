...
def cadFapt(request):
    members = Fapt.objects.all()
    context2 = {'members': members}
    return render(request, 'crud/cadFapt.html', context2)

def createFapt(request):

    member=Fapt(TipoDePublicacao = request.POST['TipoDePublicacao'], NumeroDaPublicacao = request.POST['NumeroDaPublicacao'], Emissor = request.POST['Emissor'], DescAplicabilidade = request.POST['DescAplicabilidade'], Acao = request.POST['Acao'], DataLimite  = request.POST['DataLimite '],)
    member.save()
    return redirect('/')


def editFapt(request, id):
    members = Fapt.objects.get(id=id)
    context = {'members': members}
    return render(request, 'crud/editFapt.html', context)

def updateFapt(request, id):
    member = Fapt.objects.get(id=id)
    member.TipoDePublicacao=request.POST['TipoDePublicacao']
    member.NumeroDaPublicacao=request.POST['NumeroDaPublicacao']
    member.Revisao=request.POST['Revisao']
    member.DataEfetividade=request.POST['DataEfetividade']
    member.DocAssociado=request.POST['DocAssociado']
    member.Emissor=request.POST['Emissor']
    member.ATA=request.POST['ATA']
    member.Assunto=request.POST['Assunto']
    member.Categoria=request.POST['Categoria']
    member.Tipo=request.POST['Tipo']
    member.Aplicabilidade=request.POST['Aplicabilidade']
    member.DescAplicabilidade=request.POST['DescAplicabilidade']
    member.Acao=request.POST['Acao']
    member.Analista=request.POST['Analista']
    member.TSN=request.POST['TSN']
    member.DataAnalise=request.POST['DataAnalise']
    member.TipoDeControle=request.POST['TipoDeControle']
    member.CSN=request.POST['CSN']
    member.DataLimite=request.POST['DataLimite']
    member.Justificativa=request.POST['Justificativa']
    member.save()
    return redirect('/crud/')

def deleteFapt(request, id):
    member = Fapt.objects.get(id=id)
    member.delete()
    return redirect('/crud/')